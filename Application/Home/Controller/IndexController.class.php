...
class IndexController extends BaseController {
    ...
    public function _initialize()
    {
        ...
        $doc = M('his_doctors')->where("mobile='$phone'")->find();
        $pic = "http://".$_SERVER['HTTP_HOST'].'/'.$res['pic'];
        $this->assign('pic',$pic);
        $this->assign('username',$res['username']);
        $this->assign('docname',$doc['username']);
...
function getCity($ip = '')
{
     ...
     public function docreset() 
    {
        ...
        $res = M('his_doctors')->where("mobile='$mobile'")->save($data);
  ...
  public function profilepassword()
  {
   ...
   public function resetpassword() 
    ...
    public function docLogin()
    {
      ...
      $user = M('his_doctors')->where(['mobile'=>$mobile])->find();
      ...
      $res = M('his_doctors')->where(['mobile'=>$mobile])->save(['updatetime'=>NOW_TIME]);
        if (!$res) {
            $this->error('登录失败，请重新登录');
        }
